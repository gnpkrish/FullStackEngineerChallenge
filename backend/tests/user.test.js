const debug = require('debug')('ers:user.test.js');
...
describe('user/', () => {
  ...
  it('/user/register - OK', (done) => {
    agent
      ...
      .then(({ body }) => {
        ...
        expect(body).toContainAllKeys(['id', 'email', 'role']);
  ...
  it('/graphql:M register - OK', async (done) => {
    const { body } = await agent
      ...
      .send({
        query: `mutation  {
            register( data: { firstName: "Test Name", email: "${email2}", password: "${password}" }) {
                email
                role
                id
            }
    ...
    expect(register).toContainAllKeys(['email', 'role', 'id']);
    expect(register.email).toBe(email2);
    expect(register.role).toBe('Employee');
    expect(register.id).toBeNonEmptyString();
    done();
  });
  ...
  it('/user/login - OK to login first time', (done) => {
    agent
      ...
      .then(({ body, headers }) => {
        ...
        expect(body).toContainAllKeys(['email', 'role', 'id']);
        expect(body.email).toBe(email);
        expect(body.role).toBe('Employee');
        expect(body.id).toBeNonEmptyString();
        done();
      });
  });
